...
public class ServiceBuilderContentDescriber extends XMLContentDescriber implements IExecutableExtension {
	...
	private int checkCriteria(InputSource contents)
		...
		ServiceBuilderContentHandler contentHandler = new ServiceBuilderContentHandler();
		...
		// Check to see if we matched our criteria.
		if (contentHandler.hasServiceBuilderDTD()) {
			if (contentHandler.hasTopLevelServiceElement()) {
				return VALID;
			}
