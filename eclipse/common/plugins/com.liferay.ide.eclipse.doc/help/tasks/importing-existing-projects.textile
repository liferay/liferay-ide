h1. Importing Existing Projects into Liferay IDE
...
# In Eclipse, go to File > New > Project... > Liferay > Liferay Project from Existing Source !existing-project-wizard-shortcut.png! Or you can invoke the same wizard from the Liferay shortcut toolbar !images/existing-project-wizard-shortcut-toolbar.png! 
# Browse to the location of the project folder.  **Note** the project folder should be a sub-directory of one of the plugin types, e.g. portlets, hooks, themes, etc.  If not it will not be able to be imported correctly. !images/existing-project-browse.png!
# Once you select the plugin project folder you should see the plugin type and SDK version values get updated to correct values.  If the SDK is not recent enough or project type is not correct it will be marked with error !images/existing-project-sdk-version.png!
# Next you will need to select a Liferay Runtime to configure on the project once it is imported. If you don't have a Liferay Runtime, use the New... button to create a new Liferay portal runtime (tomcat bundle only supported) !images/existing-project-select-runtime.png!
...
# In Eclipse go to File > Import... > Liferay > Liferay Plugin SDK projects !images/existing-project-sdk-projects.png!
# First you must select the Plug-ins SDK that you want to import projects from in the combo box at the top of the wizard !images/existing-project-sdk-projects-wizard.png!
...
# Once you have a configured Plug-ins SDK, you can select it in the Combo box and then SDK location and version will be filled in.  If either are not valid it will be marked with an error. !images/existing-project-sdk-projects-wizard-2.png!
# After the SDK is selected the list of projects that are available for import will be shown in the table.  If the projects are already in the workspace they will be disabled.  If the project is available for import it will have a empty checkbox that can be selected.  !images/existing-project-sdk-projects-wizard-3.png!
# Select which projects that you wish to import.
# Select the Liferay runtime that you want to setup for the imported projects
# **Note** if you don't have a liferay runtime you can add one with the "New..." button. !images/existing-project-select-runtime.png!
# Click Finish
# See the section "Verify Correct Project Configuration":#verify to verify that your project was configured correctly as a Liferay IDE project.
...
# In Eclipse, right click the eclipse project that you want to convert, select Liferay > Convert to Liferay plug-in project. !images/existing-project-convert-action.png!
...
# When the convert dialog wizard opens your project should be auto-selected and the SDK location and SDK version should be auto-detected.  If they are not valid an error message will be displayed. !images/existing-project-convert-wizard.png!
# Select the Liferay runtime that you wish to set on the project. !images/existing-project-select-runtime.png!
...
# Once the project import process is finished, you should see a new project inside Eclipse and it should have a "L" overlay image to show its a Liferay project.  !images/existing-project_overlay.png!
# Secondly, to make sure the project is now a "Liferay IDE" project is to check the target runtime project property (right-click project > properties > target runtimes) and also check the project facets to make sure both Liferay runtime and Liferay plug-in facets are properly configured. !images/existing-project_runtime.png! !images/existing-project_facets.png!