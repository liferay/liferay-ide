...
public class MarkDoneAction extends ProblemAction {
	...
	public void run(List<UpgradeProblem> upgradeProblems) {
		...
		stream.forEach(upgradeProblem -> upgradeProblem.setStatus(UpgradeProblem.STATUS_RESOLVED));
	...
	protected IStatus runWithMarker(UpgradeProblem upgradeProblem) {
		...
		try {
			if (marker.exists()) {
				...
				marker.setAttribute("migrationProblem.resolved", Boolean.TRUE);
			}

			upgradeProblem.setStatus(UpgradeProblem.STATUS_RESOLVED);
		}
		catch (CoreException ce) {
			retval = UpgradeProblemsUIPlugin.createErrorStatus("Unable to mark done the problem");
		}
