 ...
 */
@Component(
	property = "id=" + CopyPluginsSDKProjectsToWorkspaceCommandKeys.ID, scope = ServiceScope.PROTOTYPE,
	service = UpgradeCommand.class
)
public class CopyPluginsSDKProjectsToWorkspaceCommand implements UpgradeCommand, SapphireContentAccessor {
	...
	public IStatus perform(IProgressMonitor progressMonitor) {
		...
		if (FileUtil.notExists(pluginsSDKLoaction.toFile())) {
			return UpgradeCommandsUIPlugin.createErrorStatus(
				"There is no plugins-sdk folder in " + targetProjectLocation);
		}
			...
			List<IProject> projects = stream.map(
				...
				source -> {
					...
					try {
						...
						UpgradeCommandsUIPlugin.logError(
