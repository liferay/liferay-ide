    ...
    implements IWorkbenchWizard, INewWizard
{
    ...
    protected void performPostFinish()
    {
        ...
        openLiferayPerspective( projects.get(0) );

        showInAntView( projects.get(0) );

        if( projects.get(0) != null && projects.get(0).getFile( ISDKConstants.IVY_XML_FILE ).exists() )
        {
            new WorkspaceJob( "Configuring project with Ivy dependencies" ) //$NON-NLS-1$
            {
                public IStatus runInWorkspace( IProgressMonitor monitor ) throws CoreException
                {
                    try
                    {
                        IvyUtil.configureIvyProject( projects.get(0), monitor );
                    }
        ...
        if( createNewPortlet && PluginType.portlet.equals( op.getPluginType().content() ) )
        {
            ...
            if( wizardId != null )
            {
                openNewPortletWizard( wizardId, projects.get(0) );
            }
        }
    }
