 ...
 * @author Terry Jia
 */
public class UpgradeStepCategoryValueImageService extends ValueImageService implements UpgradePlanAcessor {
		...
		return _imageMap.computeIfAbsent(
			...
			v -> {
				return Optional.ofNullable(
					...
					UpgradeStepCategory::getImagePath
				).filter(
					Objects::nonNull
				).map(
					imagePath -> ImageData.readFromClassLoader(UpgradeStepCategoryValueImageService.class, imagePath)
				).map(
					Result::required
				).orElse(
					null
