...
public class DdmTemplateSelectorTagsAutoCorrectTest {

	@Test
	public void autoCorrectProblems() throws Exception {
		File tempFolder = Files.createTempDirectory("autocorrect").toFile();
		File testFile = new File(tempFolder, "DdmTemplateSelectorTagsTest.jsp");

		tempFolder.deleteOnExit();

		File originalTestfile = new File("jsptests/ddm-template-selector/DdmTemplateSelectorTagsTest.jsp");
		...
		for (ServiceReference<FileMigrator> mref : mrefs) {
			...
			if (migrator.getClass().getName().contains("DdmTemplateSelectorTags")) {
