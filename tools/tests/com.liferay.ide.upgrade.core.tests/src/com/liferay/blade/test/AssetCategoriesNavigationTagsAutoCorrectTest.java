...
public class AssetCategoriesNavigationTagsAutoCorrectTest {

	@Test
	public void autoCorrectProblems() throws Exception {
		File tempFolder = Files.createTempDirectory("autocorrect").toFile();
		File testFile = new File(tempFolder, "AssetCategoriesNavigationTagsTest.jsp");

		tempFolder.deleteOnExit();

		File originalTestfile =
			new File("jsptests/asset-categories-navigation/AssetCategoriesNavigationTagsTest.jsp");
		...
		for (ServiceReference<FileMigrator> mref : mrefs) {
			...
			if (migrator.getClass().getName().contains("AssetCategoriesNavigationTags")) {
