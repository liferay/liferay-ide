 ...
 * @author Vicky Wang
 */
public class ContainerSelectionPageObject<T extends SWTBot> extends DialogPageObject<T>
    implements HookConfigurationWizard, ProjectWizard
{

    TreePageObject<SWTBot> path;
    TextPageObject<SWTBot> validationMessage;

    public ContainerSelectionPageObject( T bot )
    {
        this( bot, TEXT_BLANK );
    }

    public ContainerSelectionPageObject( T bot, String title )
    {
        super( bot, title, BUTTON_CANCEL, BUTTON_OK );

        path = new TreePageObject<>( bot );
        validationMessage = new TextPageObject<SWTBot>( bot );
    }
