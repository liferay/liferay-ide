...
public class LogoSelectorTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/logo-selector/"), versions, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("LogoSelectorTagsTest.jsp") && (problem.getLineNumber() == 11) &&
				(problem.getStartOffset() >= 595) && (problem.getEndOffset() >= 606)) {
