...
public class AUIButtonItemTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/aui-button/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("AUIButtonItemTagTest.jsp")) {
				if ((problem.getLineNumber() == 3) && (problem.getStartOffset() >= 94) && (problem.getEndOffset() >= 129)) {
					found = true;
				}
			}
		}
