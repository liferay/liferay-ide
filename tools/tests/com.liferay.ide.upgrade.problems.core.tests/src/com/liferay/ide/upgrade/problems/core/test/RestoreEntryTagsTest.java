...
public class RestoreEntryTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/restore-entry/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("RestoreEntryTagsTest.jsp")) {
				if ((problem.getLineNumber() == 2) && (problem.getStartOffset() >= 12) && (problem.getEndOffset() >= 318)) {
					found = true;
				}
			}
		}
