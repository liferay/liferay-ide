...
public class NavigationTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/navigation-tags/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("NavigationTagsTest.jsp")) {
				if ((problem.getLineNumber() == 3) && (problem.getStartOffset() >= 57) && (problem.getEndOffset() >= 374)) {
					found = true;
				}
			}
		}
