...
public class DeprecatedLiferaySecurityEncryptTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(
			new File("jsptests/liferay-security-encrypt/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("LiferaySecurityEncrypt.jsp")) {
