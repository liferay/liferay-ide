...
public class AssetCategoriesNavigationTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(
			new File("jsptests/asset-categories-navigation/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("AssetCategoriesNavigationTagsTest.jsp")) {
				if ((problem.getLineNumber() == 48) && (problem.getStartOffset() >= 1530) && (problem.getEndOffset() >= 1616)) {
					found = true;
				}
			}
		}
