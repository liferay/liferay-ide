...
public class JournalArticleTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/journal-article-tags/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());

		Assert.assertEquals("", 1, problems.size());

		UpgradeProblem problem = problems.get(0);

		Assert.assertTrue(problem.getResource().getName().endsWith("JournalArticleTagsTest.jsp"));

		Assert.assertEquals("", 4, problem.getLineNumber());

		Assert.assertEquals("", "LPS-69321", problem.getTicket());
	}
