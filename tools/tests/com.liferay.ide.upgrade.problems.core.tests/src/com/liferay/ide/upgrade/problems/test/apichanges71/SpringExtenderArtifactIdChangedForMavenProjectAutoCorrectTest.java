 ...
 * @author Seiphon Wang
 */
public class SpringExtenderArtifactIdChangedForMavenProjectAutoCorrectTest {

	@Test
	public void autoCorrectProblems() throws Exception {
		File tempFolder = Files.createTempDirectory("autocorrect").toFile();

		File testFile = new File(tempFolder, "pom.xml");

		tempFolder.deleteOnExit();

		File originalTestfile = new File("tests/files/pom.xml");
		...
		for (ServiceReference<FileMigrator> mref : mrefs) {
			...
			if (clazz.getName().contains("SpringExtenderArtifactIdChangedForMavenProject")) {
