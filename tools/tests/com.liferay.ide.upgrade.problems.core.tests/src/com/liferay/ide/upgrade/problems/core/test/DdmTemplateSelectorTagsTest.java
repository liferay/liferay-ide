...
public class DdmTemplateSelectorTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/ddm-template-selector/"),UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("DdmTemplateSelectorTagsTest.jsp")) {
				if ((problem.getLineNumber() == 70) && (problem.getStartOffset() >= 2538) && (problem.getEndOffset() >= 2549)) {
					found = true;
				}
			}
		}
