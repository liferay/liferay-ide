...
public class AUILiferayTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(new File("jsptests/aui-liferay/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("AUILiferayTagTest.jsp")) {
				if ((problem.getLineNumber() == 1) && (problem.getStartOffset() == 16) && (problem.getEndOffset() == 48)) {
					found = true;
				}
			}
		}
