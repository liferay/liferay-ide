...
public class DeprecatedLiferayPortletIconBackTagsTest {
	...
	public void findUpgradeProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(
			new File("jsptests/liferay-portlet-icon-back/"), UpgradePlanProblemsTestConstants.TEST_VERSIONS, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if (problem.getResource().getName().endsWith("LiferayPortletIconBack.jsp")) {
